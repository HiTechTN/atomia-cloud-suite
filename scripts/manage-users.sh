...
USER_PROXIES="./data/nginx/user_proxies.conf"
...
EOF
    ...
    mkdir -p "$(dirname "$USER_PROXIES")"
    cat > "$USER_PROXIES" <<EOF
# Auto-generated by manage-users.sh
$(grep -oP 'code-server-\w+:' "$USERS_COMPOSE" | sed 's/://' | while read -r service; do
    user=${service#code-server-}
    port=$(grep -A5 "$service" "$USERS_COMPOSE" | grep -oP '(?<=")\d+(?=:8080")')
    echo "location /u/$user/ {"
    echo "    proxy_pass http://host.docker.internal:$port/;"
...
EOF
    ...
    ok "Proxy configuration updated at $USER_PROXIES"
